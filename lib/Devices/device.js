...
module.exports = class MiioAdapterDevice extends EventEmitter {
    constructor(miioDev) {
        ...
        this.miioDev.propertyUpdated = this.propertyUpdated;
    }
    ...
    invokeCommand(cmd, val) {
        ...
        if (valType !== cmd.para[0].type) {
            this.emit("warning", `value type ${valType} dose not match to command parameter type ${cmd.para[0].type}.`);
            return;
        }
        ...
        this.miioDev.call(cc, cvl)
            .then(this.miioDev.checkOk);
    }

    propertyUpdated(prop, value, oldValue) {
        // Value here is already mapped.
        if (this.props[prop]) {
            this.attributeUpdate(prop, value, oldValue);
        }
    }
    ...
    listen(props) {
        ...
        for (const k in props) {
            if (props.hasOwnProperty(k)) {
                const prop = props[k];
                this.miioDev.defineProperty(prop.prop, (v) => {
                    if (prop.obj) {
                        return prop.obj[v];
                    }
                    if ((typeof v === "string") && (prop.type === "number"))
                        return +v;
                });
            }
        }
