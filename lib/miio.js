...
module.exports = class Controller extends EventEmitter {
    ...
    listen() {
        ...
        for (let i = 0; i < this.devicesDefined.length; i++) {
            ...
            miio.device({
                address: dev.ip,
                token: dev.token
            }).then(/** @param {AdapterMiio.MiioLibDevice} dev */ dev => {
        ...
        if (this.autoDiscover) {
            ...
            browser.on("available", /** @param {AdapterMiio.MiioLibRegisterInfo} reg */ reg => {
                if (!reg.token) {
            ...
            browser.on("unavailable", /** @param {AdapterMiio.MiioLibRegisterInfo} reg */ reg => {
    ...
    /**
     * 
     * @param {AdapterMiio.MiioLibDevice} dev 
     * @param {string} vendor 
     * @param {string} type 
