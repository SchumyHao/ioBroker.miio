    ...
    listen() {
        ...
        for (let i = 0; i < this.devicesDefined.length; i++) {
            ...
            miio.device({
                ...
                dev.on("thing:destroyed", () => {
                    ...
                    this.emit("warning", `thing:destroyed`);
                    if (!miioID) {
                        this.emit("info", "Cannot remove device without Device ID");
                        return;
                    }
                    const regDev = this.deviceRegistered[miioID];
                    if (!regDev) return;

                    this.emit("device", this.deviceRegistered[miioID], "delete");
                    delete this.deviceRegistered[miioID];
                    this.emit("warning", `${miioID} becomes unavailable.`);
        ...
        if (this.autoDiscover) {
            ...
            browser.on("unavailable", /** @param {miio.RegisterInfo} reg */ reg => {
                ...
                this.emit("device", this.deviceRegistered[reg.id], "delete");
